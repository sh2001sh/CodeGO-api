...
func TestShouldSkipClaudeMessageDeltaUsagePatch(t *testing.T) {
	originGlobalPassThrough := gatewaystore.GetGlobalSettings().PassThroughRequestEnabled
	t.Cleanup(func() {
		gatewaystore.GetGlobalSettings().PassThroughRequestEnabled = originGlobalPassThrough
	})

	gatewaystore.GetGlobalSettings().PassThroughRequestEnabled = true
	assert.True(t, shouldSkipClaudeMessageDeltaUsagePatch(&relaycommon.RelayInfo{}))

	gatewaystore.GetGlobalSettings().PassThroughRequestEnabled = false
	assert.True(t, shouldSkipClaudeMessageDeltaUsagePatch(&relaycommon.RelayInfo{
