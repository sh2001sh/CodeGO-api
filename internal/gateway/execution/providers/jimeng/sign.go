...
func Sign(c *gin.Context, req *http.Request, apiKey string) error {
	...
	canonicalRequest := fmt.Sprintf("%s\n%s\n%s\n%s\n%s\n%s", method, u.Path, canonicalQueryString, canonicalHeaders.String(), signedHeaders, hexPayloadHash)
	...
	authorization := fmt.Sprintf("HMAC-SHA256 Credential=%s/%s, SignedHeaders=%s, Signature=%s", accessKey, credentialScope, signedHeaders, signature)
