...
type RelayInfo struct {
	...
	// 免费模型时为 nil。
	Billing gatewaycontract.BillingSettler
	...
	// internal/gateway/runtime/outbound_body.go), so that DoApiRequest can populate
	...
	StreamStatus *gatewaycontract.StreamStatus
...
func (info *RelayInfo) InitChannelMeta(c *gin.Context) {
	channelType := httpctx.GetContextKeyInt(c, constant.ContextKeyChannelType)
	paramOverride := httpctx.GetContextKeyStringMap(c, constant.ContextKeyChannelParamOverride)
	...
	channelSetting, ok := httpctx.GetContextKeyType[dto.ChannelSettings](c, constant.ContextKeyChannelSetting)
	if ok {
		channelMeta.ChannelSetting = channelSetting
	}

	channelOtherSettings, ok := httpctx.GetContextKeyType[dto.ChannelOtherSettings](c, constant.ContextKeyChannelOtherSetting)
	if ok {
...
func (info *RelayInfo) ToString() string {
	...
	fmt.Fprintf(b, "User{ Id: %d, Email: %q, Group: %q, UsingGroup: %q, Quota: %d }, ",
		info.UserId, platformtext.MaskEmail(info.UserEmail), info.UserGroup, info.UsingGroup, info.UserQuota)
...
func GenRelayInfoRerank(c *gin.Context, request *dto.RerankRequest) *RelayInfo {
	info := genBaseRelayInfo(c, request)
	info.RelayMode = gatewaycontract.RelayModeRerank
...
func GenRelayInfoResponses(c *gin.Context, request *dto.OpenAIResponsesRequest) *RelayInfo {
	...
	if len(request.Tools) > 0 {
		for _, tool := range request.GetToolsMap() {
			toolType := platformencoding.Interface2String(tool["type"])
			...
			case dto.BuildInToolWebSearchPreview:
				searchContextSize := platformencoding.Interface2String(tool["search_context_size"])
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	// 当令牌分组为空时，表示使用用户分组
	if tokenGroup == "" {
		tokenGroup = httpctx.GetContextKeyString(c, constant.ContextKeyUserGroup)
	}
	...
	reqId := httpctx.GetContextKeyString(c, constant.RequestIdKey)
	if reqId == "" {
		reqId = platformruntime.GetTimeString() + platformruntime.GetRandomString(8)
	}
	info := &RelayInfo{
		Request: request,
		...
		UserQuota:  httpctx.GetContextKeyInt(c, constant.ContextKeyUserQuota),
		UserEmail:  httpctx.GetContextKeyString(c, constant.ContextKeyUserEmail),
		...
		TokenCountMeta: TokenCountMeta{
			//promptTokens: httpctx.GetContextKeyInt(c, constant.ContextKeyPromptTokens),
			estimatePromptTokens: httpctx.GetContextKeyInt(c, constant.ContextKeyEstimatedTokens),
		},
	}

	if info.RelayMode == gatewaycontract.RelayModeUnknown {
	...
	userSetting, ok := httpctx.GetContextKeyType[dto.UserSetting](c, constant.ContextKeyUserSetting)
	if ok {
...
type TaskRelayInfo struct {
	...
	// to avoid an import cycle with the gateway schema; callers type-assert to *schema.Channel.
	LockedChannel any
}
...
func (t *TaskSubmitReq) UnmarshalJSON(data []byte) error {
	...
	if len(aux.Duration) > 0 {
		var durationInt int
		if err := platformencoding.Unmarshal(aux.Duration, &durationInt); err == nil {
			t.Duration = durationInt
		} else {
			var durationStr string
			if err := platformencoding.Unmarshal(aux.Duration, &durationStr); err == nil && durationStr != "" {
	...
	if len(aux.Metadata) > 0 {
		...
		if err := platformencoding.Unmarshal(aux.Metadata, &metadataStr); err == nil && metadataStr != "" {
			var metadataObj map[string]interface{}
			if err := platformencoding.Unmarshal([]byte(metadataStr), &metadataObj); err == nil {
		...
		var metadataObj map[string]interface{}
		if err := platformencoding.Unmarshal(aux.Metadata, &metadataObj); err == nil {
			t.Metadata = metadataObj
		}
	}
...
func (t *TaskSubmitReq) UnmarshalMetadata(v any) error {
	metadata := t.Metadata
	if metadata != nil {
		metadataBytes, err := platformencoding.Marshal(metadata)
		if err != nil {
			return fmt.Errorf("marshal metadata failed: %w", err)
		}
		err = platformencoding.Unmarshal(metadataBytes, v)
		if err != nil {
...
func RemoveDisabledFields(jsonData []byte, channelOtherSettings dto.ChannelOtherSettings, channelPassThroughEnabled bool) ([]byte, error) {
	...
	jsonDataAfter, err := platformencoding.Marshal(data)
	if err != nil {
		platformobservability.SysError("RemoveDisabledFields Marshal error :" + err.Error())
		return jsonData, nil
	}
...
func RemoveGeminiDisabledFields(jsonData []byte) ([]byte, error) {
	...
	var data map[string]interface{}
	if err := platformencoding.Unmarshal(jsonData, &data); err != nil {
		platformobservability.SysError("RemoveGeminiDisabledFields Unmarshal error: " + err.Error())
		return jsonData, nil
	}
	...
	jsonDataAfter, err := platformencoding.Marshal(data)
	if err != nil {
		platformobservability.SysError("RemoveGeminiDisabledFields Marshal error: " + err.Error())
		return jsonData, nil
	}
	return jsonDataAfter, nil
}
