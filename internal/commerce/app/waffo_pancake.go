...
func CreateWaffoPancakeCheckoutSession(ctx context.Context, params *WaffoPancakeCreateSessionParams) (*WaffoPancakeCheckoutSession, error) {
	...
	body, err := platformencoding.Marshal(params)
	if err != nil {
		return nil, fmt.Errorf("marshal Waffo Pancake checkout payload: %w", err)
	}

	privateKey, err := normalizeRSAPrivateKey(commercestore.WaffoPancakePrivateKey)
	if err != nil {
		return nil, err
	}
	...
	req.Header.Set("X-Merchant-Id", commercestore.WaffoPancakeMerchantID)
	...
	if err := platformencoding.Unmarshal(responseBody, &result); err != nil {
...
func ResolveWaffoPancakeTradeNo(event *waffoPancakeWebhookEvent) (string, error) {
	...
	if tradeNo := strings.TrimSpace(event.Data.OrderID); tradeNo != "" {
		...
		if topUp != nil && topUp.PaymentMethod == commerceschema.PaymentMethodWaffoPancake {
			return tradeNo, nil
		}
...
func verifyWaffoPancakeWebhook(payload string, signatureHeader string, environment string) (*waffoPancakeWebhookEvent, error) {
	...
	if err := platformencoding.Unmarshal([]byte(payload), &event); err != nil {
...
func resolveWaffoPancakeWebhookEnvironment(payload string) string {
	...
	if err := platformencoding.Unmarshal([]byte(payload), &envelope); err != nil {
		if commercestore.WaffoPancakeSandbox {
			return "test"
		}
		return "prod"
...
func resolveWaffoPancakeWebhookPublicKey(environment string) string {
	if environment == "prod" {
		return strings.TrimSpace(commercestore.WaffoPancakeWebhookPublicKey)
	}
	return strings.TrimSpace(commercestore.WaffoPancakeWebhookTestKey)
}
