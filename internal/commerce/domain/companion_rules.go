...
const companionMaxDiscountRate = 0.8
...
func BuildCompanionPetBuff(achievementKey string, level int) CompanionPetBuff {
	...
	buff := CompanionPetBuff{
		...
		CheckinBonusQuota:          int64(math.Round(companionFloatAt(blueprint.CheckinBonusUSD, level) * platformruntime.QuotaPerUnit)),
		BlindBoxBonusQuota:         int64(math.Round(companionFloatAt(blueprint.BlindBoxBonusUSD, level) * platformruntime.QuotaPerUnit)),
...
func usdTextFromQuota(quota int64) string {
	return fmt.Sprintf("%.2f 美元", float64(quota)/platformruntime.QuotaPerUnit)
}
...
func buildCompanionBuffTexts(buff CompanionPetBuff) (string, string) {
	...
	if buff.BlindBoxPityReduction > 0 {
		effectiveThreshold := 5 - buff.BlindBoxPityReduction
