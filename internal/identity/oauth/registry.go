...
func LoadCustomProviders() error {
	...
	customProviders, err := listCustomOAuthProviders()
	if err != nil {
	...
	for _, config := range customProviders {
		...
		platformobservability.SysLog("Loaded custom OAuth provider: " + config.Name + " (" + config.Slug + ")")
	}

	platformobservability.SysLog(fmt.Sprintf("Loaded %d custom OAuth providers", len(customProviders)))
	return nil
}
...
func RegisterOrUpdateCustomProvider(config *identitydomain.CustomOAuthProvider) {
	provider := NewGenericOAuthProvider(config)
	mu.Lock()
	defer mu.Unlock()
