...
type GenericOAuthProvider struct {
	config *identitydomain.CustomOAuthProvider
}
...
func NewGenericOAuthProvider(config *identitydomain.CustomOAuthProvider) *GenericOAuthProvider {
...
func (p *GenericOAuthProvider) GetConfig() *identitydomain.CustomOAuthProvider {
	return p.config
}
...
func (p *GenericOAuthProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	redirectUri := fmt.Sprintf("%s/oauth/%s", platformconfig.ServerAddress, p.config.Slug)
	values := url.Values{}
	...
	if err := platformencoding.Unmarshal(body, &tokenResponse); err != nil {
...
func (p *GenericOAuthProvider) IsUserIDTaken(providerUserID string) bool {
	return identitystore.IsOAuthProviderUserIDTaken(p.config.Id, providerUserID)
}

func (p *GenericOAuthProvider) FillUserByProviderID(user *identityschema.User, providerUserID string) error {
	foundUser, err := identitystore.LoadUserByOAuthBinding(p.config.Id, providerUserID)
	if err != nil {
		return err
	}
	*user = *foundUser
...
func (p *GenericOAuthProvider) SetProviderUserID(user *identityschema.User, providerUserID string) {
...
func parseAccessPolicy(raw string) (*accessPolicy, error) {
	...
	if err := platformencoding.UnmarshalString(raw, &policy); err != nil {
		return nil, err
	}
...
func gjsonResultToValue(result gjson.Result) any {
	...
	case gjson.JSON:
		var data any
		if err := platformencoding.UnmarshalString(result.Raw, &data); err == nil {
			return data
		}
		return result.Raw
	default:
