...
func (p *DiscordProvider) IsEnabled() bool {
	return platformstore.GetDiscordSettings().Enabled
}
...
func (p *DiscordProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	settings := platformstore.GetDiscordSettings()
	redirectUri := fmt.Sprintf("%s/oauth/discord", platformconfig.ServerAddress)
	values := url.Values{}
...
func (p *DiscordProvider) IsUserIDTaken(providerUserID string) bool {
	return identitystore.IsDiscordIDTaken(providerUserID)
}

func (p *DiscordProvider) FillUserByProviderID(user *identityschema.User, providerUserID string) error {
	loadedUser, err := identitystore.LoadUserByDiscordID(providerUserID)
	if err != nil {
		return err
	}
	*user = *loadedUser
...
func (p *DiscordProvider) SetProviderUserID(user *identityschema.User, providerUserID string) {
