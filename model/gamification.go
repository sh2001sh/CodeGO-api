...
// CountPaidConsumeLogsByUser counts consume logs that actually deducted quota.
func CountPaidConsumeLogsByUser(userId int, startTime int64, endTime int64) (int64, error) {
	var count int64
	query := LOG_DB.Model(&Log{}).
		Where("user_id = ? AND type = ? AND quota > ?", userId, LogTypeConsume, 0)
	if startTime > 0 {
