...
import { CircleAlert, Loader2, Sparkles, WandSparkles } from 'lucide-react'
import type {
  BlindBoxCredit,
  BlindBoxSelfData,
  PaymentMethod,
    ...
    <div className='grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_360px]'>
      <div className='overflow-hidden rounded-[30px] border border-slate-200 bg-[radial-gradient(circle_at_top_left,rgba(250,204,21,0.22),transparent_30%),radial-gradient(circle_at_top_right,rgba(251,146,60,0.18),transparent_24%),linear-gradient(145deg,rgba(255,255,255,0.98),rgba(255,247,237,0.98),rgba(248,250,252,0.98))] p-4 shadow-[0_26px_90px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-[radial-gradient(circle_at_top_left,rgba(250,204,21,0.18),transparent_28%),radial-gradient(circle_at_top_right,rgba(249,115,22,0.16),transparent_24%),linear-gradient(145deg,rgba(30,20,8,0.96),rgba(15,23,42,0.96),rgba(17,24,39,0.94))]'>
        <div className='flex flex-wrap items-start justify-between gap-4'>
          <div className='max-w-2xl'>
            ...
            <p className='mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300'>
              支付完成后会留在当前页直接开奖，保底进度、剩余盒数、常规奖池和最近掉落都集中在这里。
            </p>
          </div>
          ...
          <div className='rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-medium text-amber-700 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200'>
            单盒 {props.data?.unit_price?.toFixed(1) || '0.0'} 美元
          </div>
        </div>
        ...
        <div className='mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]'>
          <div className='rounded-[28px] border border-slate-200 bg-white/82 p-4 dark:border-slate-800 dark:bg-slate-950/58'>
                ...
                <div className='flex flex-wrap items-center justify-between gap-3'>
                  <div>
                    ...
                    <div className='mt-1 text-2xl font-semibold text-slate-950 dark:text-slate-50'>
                      {props.amountDue.toFixed(2)} 美元
                    </div>
                    <div className='mt-1 text-sm text-slate-500 dark:text-slate-400'>
                      支付成功后会留在当前页自动开奖，并弹出本次掉落结果。
                    </div>
                  </div>
                  <Button
                    onClick={props.onPay}
          ...
          <div className='space-y-4'>
            <div className='rounded-[28px] border border-slate-200 bg-white/82 p-4 dark:border-slate-800 dark:bg-slate-950/58'>
              ...
              <div className='mt-1 text-sm text-slate-500 dark:text-slate-400'>
                连续开出低于 {props.data?.low_reward_threshold_usd || 0}{' '}
                美元的奖励达到门槛后，下一次至少获得{' '}
                {props.data?.pity_guarantee_usd || 0} 美元奖励。
              </div>
              <div className='mt-4'>
                <Progress
                  className='[&_[data-slot=progress-indicator]]:bg-amber-500'
                  value={props.pityPercent}
                />
              </div>
            ...
            <div className='rounded-[28px] border border-slate-200 bg-white/82 p-4 dark:border-slate-800 dark:bg-slate-950/58'>
              ...
              <div className='mt-3 space-y-3'>
                {(props.data?.tiers || []).map((tier) => (
                  <div
                    key={tier.name}
                    className='flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-3 text-sm dark:border-slate-800 dark:bg-slate-900/70'
                  >
                ...
                <div className='flex items-center justify-between gap-3 rounded-2xl border border-violet-200 bg-violet-50 px-3 py-3 text-sm dark:border-violet-500/20 dark:bg-violet-500/10'>
                  <div className='text-violet-700 dark:text-violet-200'>
                    {props.data?.subscription_plan_title || '月卡大奖'}
                  </div>
                  <div className='font-semibold text-violet-700 dark:text-violet-200'>
                    {(
      ...
      <div className='space-y-4'>
        <div className='rounded-[30px] border border-slate-200 bg-card p-4 shadow-xs dark:border-slate-800'>
          ...
          <div className='mt-1 text-sm text-slate-500 dark:text-slate-400'>
            盲盒额度会优先于订阅和钱包余额消耗，适合短期冲量、补库存或集中试用高价模型。
          </div>
