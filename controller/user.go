...
func GetAffiliateRewardsOverview(c *gin.Context) {
	id := c.GetInt("id")
	user, err := model.GetUserById(id, true)
	if err != nil {
		common.ApiError(c, err)
	...
	overview, err := service.GetAffiliateRewardsOverview(id)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func TopUp(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, model.ErrRedemptionBusy):
			common.ApiErrorI18n(c, i18n.MsgRedemptionBusy)
		default:
			common.ApiErrorI18n(c, i18n.MsgRedemptionBusy)
		}
		return
...
func UpdateUserSetting(c *gin.Context) {
	...
	// 在现有设置上增量更新，避免覆盖掉订阅扣费偏好等其他独立入口写入的字段。
